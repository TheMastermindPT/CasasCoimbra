...
$(window).ready(() => {
    ...
    $.ajax({
        ...
        data.forEach((casa, index) => {
            ...
            if (index < 3) {
                ...
                let children = $('.imoveis__wrapper').eq(times).children().length;
                if (children < 3) {
                    template(times, casa);
                }
            } else {
            ...
            $('.imovel__info').eq(index).append(`<button class="conheca">Conheça</button>`);

        });


        //Appends X amount of placeholders depending on X of missing children
        $('.imoveis__wrapper').each((index, value) => {
            let children = $('.imoveis__wrapper').eq(index).children().length;
            ...
            if (children < 3) {
                for (let i = 0; i < numberOpenSlots(children); i++) {
                    $('.imoveis__wrapper').eq(index).append(`<div class="imovel--placeholder"></div> `)
                }
            }
        });
    });
    ...
    // Placeholder Imovel
    $.each(imoveisWrapper, (index, val) => {
        let imoveisNumber = imoveisWrapper.eq(index).children().length;
    ...
    $('#navi-toggle').on('change', () => {
        ...
        if (state) {
            $('#navigator').switchClass('navigator--closed', 'navigator', 250, 'linear');

            setTimeout(() => {
                $('.sidenav__list').css('display', 'block');
            }, 400);
        } else {
            $('#navigator').switchClass('navigator', 'navigator--closed', 250, 'linear');
            $('.sidenav__list').css('display', 'none');
        }
    });
